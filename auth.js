...
function showWelcome(){
  ...
  // Safety: if welcome page not in HTML, go straight to app
  if(!wlc){ 
  ...
  // (no greeting text - removed by design)

  const isManager=CP.role==='manager';
  const apps=[
    {icon:WLC_ICONS.dashboard, label:'لوحة التحكم',   page:'dashboard',      show:isManager},
    {icon:WLC_ICONS.visit,     label:'تسجيل زيارة',   page:'visit',          show:true},
    {icon:WLC_ICONS.plan,      label:'الخطة الشهرية', page:'plan',           show:true},
    {icon:WLC_ICONS.customers, label:'العملاء',        page:'customers',      show:true},
    {icon:WLC_ICONS.products,  label:'المنتجات',       page:'products',       show:true},
    {icon:WLC_ICONS.reports,   label:'التقارير',       page:'report-company', show:isManager},
    {icon:WLC_ICONS.users,     label:'المستخدمون',     page:'users',          show:isManager},
    {icon:WLC_ICONS.setup,     label:'الإعدادات',      page:'setup',          show:true},
  ].filter(a=>a.show);
...
function enterApp(page){
  document.getElementById('page-welcome').classList.remove('active');
  document.getElementById('app').style.display='block';
  showPage(page||( CP.role==='manager'?'dashboard':'visit'));
}
...
function buildMenubar(){
  ...
  const menuData=[
    {label:'الرئيسية',pages:[{page:'dashboard',label:'لوحة التحكم',icon:icons.dashboard}]},
    ...
    ...(isManager?[
      ...
      {label:'الإعدادات',pages:[{page:'setup',label:'إعداد الخطة',icon:icons.setup}]},
    ])
  ];
  ...
  menuData.forEach(m=>{
    ...
    // Prevent button click from bubbling to document (which calls closeDds)
    btn.addEventListener('click',e=>e.stopPropagation());

    // Logo click → welcome page (NOT logout)
    div.appendChild(btn);
    div.appendChild(dd);
